...
import { EntityArrayResponseType, PieceJointeService } from '../service/piece-jointe.service';
import { PieceJointeDeleteDialogComponent } from '../delete/piece-jointe-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'reclamation-piece-jointe',
...
export class PieceJointeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPieceJointe): number => this.pieceJointeService.getPieceJointeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.pieceJointes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPieceJointe[] | null): IPieceJointe[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.pieceJointeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
