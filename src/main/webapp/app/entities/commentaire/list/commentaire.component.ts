...
import { EntityArrayResponseType, CommentaireService } from '../service/commentaire.service';
import { CommentaireDeleteDialogComponent } from '../delete/commentaire-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'reclamation-commentaire',
...
export class CommentaireComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICommentaire): number => this.commentaireService.getCommentaireIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.commentaires = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICommentaire[] | null): ICommentaire[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.commentaireService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
