...
@Component({
  template: `
    <table>
      <thead>
        <tr reclamationSort [(predicate)]="predicate" [(ascending)]="ascending" (sortChange)="transition($event)">
          <th reclamationSortBy="name">
            ID
            @if (sortAllowed) {
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate differs from column predicate', () => {
    ...
    // THEN
    expect(sortByDirective.reclamationSortBy).toEqual('name');
